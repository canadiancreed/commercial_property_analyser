...
│       ├── financing_robustness.py # Rate/amort break-even margins, break-even diagnostic, scored robustness factor
...
| `financing.json` | Global financing assumptions, loaded by `analysis/financing_config.py`. **Two layers:** `defaults` (the four house-wide scalars — `down_payment_pct`, `interest_rate`, `term_years`, `hold_years` — edited via main menu `f`) plus an optional `property_types` map that refines down payment (`max_ltv`), rate (`rate_low`/`rate_high` midpoint), amortization (`amort_years`), DSCR floor, and fixed-expense fraction **per asset class** (office, retail, industrial, mixed-use, hotel, multi_family_1_4, multi_family_5plus, and the `retail_office → office` alias). `hold_years` is house-wide only. CMHC MLI-eligible classes — 5+ unit multifamily, and mixed-use meeting the 30% non-residential rule — rank on **MLI Standard** at an **85% acquisition-LTV cap** and carry an **MLI Select** upside sub-scenario; a `cmhc` block holds the verified MLI constants (`acquisition_ltv_cap`, `unverified_max_ltv`, `mli_standard`, `mli_select` with `amort_by_points` / `modeled_amort_years`). A uniform small-balance floor routes any deal whose 75%-LTV loan is below `min_practical_loan` (~$1M) to conventional. Missing `defaults` keys are a hard error; a missing/empty `property_types` map reproduces pre-per-type behavior exactly. |
...
| `financing_config.py` | Loads/validates `config/financing.json` (two-layer). **`get_financing(property_type, units, loan_estimate)`** / **`resolve_financing(price, …)`** merge the resolved per-type block over `defaults` and derive the four scalars the pipeline consumes. **All financing routing lives here and nowhere else:** the `retail_office → office` alias, the multi-family 1-4/5+ split by unit count, CMHC MLI eligibility (5+ residential units, or mixed-use with commercial GFA ≤30% — computed as `mixed_use_commercial_gfa_share` = 1/floors under the equal-plate convention), the **uniform small-balance carve-out** (75%-LTV loan below `min_practical_loan` → conventional, for any MLI-eligible type), and the MLI **Standard** (ranking, 85% LTV / 40yr / DSCR 1.20) + **Select** (upside, 85% cap / point-tier amortization / DSCR 1.10) scenario pair returned as `select`. Every listing is an existing-building acquisition, so LTV is capped at 85% for both programs — 95% is unmodelled, labelled upside. `load_financing_config()` still returns the `defaults` block (backward-compatible); `save_financing_config()` (menu `f`) rewrites `defaults` while preserving the `property_types` map. |
| `deal_financing.py` | **`DealFinancing`** — the additive per-type financing rows, sitting alongside `MortgageCalculator`: LTV- and DSCR-constrained loan ceilings, which one binds (`Binding Constraint`), per-door economics (`Units`, `Price / Door`, `Avg Rent / Door`), and the CMHC / MLI panel (rendered only for MLI-routed deals — 5+ multifamily or eligible mixed-use). `loan_from_payment()` inverts the semi-annual payment formula to size the DSCR-constrained loan at the qualifying rate (`contract + stress_test_bump`). |
...
| `deal_financing.py` (in `analysis/`) | Max LTV, LTV Max Loan, DSCR Max Loan (@ qualifying rate), Max Supportable Loan, Binding Constraint (`LTV` / `DSCR` / `n/a (GDS/TDS)`), and Refi Headroom (when LTV binds); for multifamily: Units, Price / Door, Avg Rent / Door. The **CMHC / MLI panel renders only when the deal is actually MLI-routed** (Standard 85% LTV / 40yr; Select 85% acquisition cap with point-tier amortization modelled at 45yr; the 95%-LTV-is-unverified-upside note) — no MLI rows appear on any conventional deal (small-balance, 1–4 unit, or non-eligible mixed-use). |
| `returns.py` | IRR (`numpy_financial.irr`), Equity Multiple, CELOC Speed Score and Seller Bleed (informational listing-economics rows — not inputs to the property score), Market Staleness (DOM). Hold period is house-wide (default **10 years**). |
| `financing_robustness.py` | **`FinancingRobustness`** — rate and amortization break-even margins to two thresholds (covenant breach and cash-flow zero) reported as two separate readings plus their spread, a break-even-financing diagnostic ("positive at X% / Yyr"), a pass/conditional/fails-all verdict, and a scored **0–100 Financing Robustness** sub-score (rate margin weighted 0.7, amortization 0.3; refs in `config/underwriting.json`). Covenant is the per-type `covenant_dscr` from `config/financing.json`. |
...
| File | Purpose |
|---|---|
| `scorer.py` | **`PropertyScorer`** — produces a 0–100 investment score from a saved property record. Weights the scored components (Cap Rate, CoCR, IRR, Equity Multiple, **Financing Robustness**, Location); **DSCR, Cash Flow, Price Drop and DOM ship at zero weight** — DSCR and Cash Flow are display-only (de-collinearized from CoCR, their weight moved to Financing Robustness), and Price Drop / DOM amplify the confidence multiplier instead (see "Market/listing signals" above). Cash Flow is scored as **% of asking price**, CoCR/Cash-Flow ramps extend below zero to preserve rank order past break-even. A stored record with no `financing_robustness` value reads as **pending** — the factor is dropped and the rest renormalize — never a silent 0. After all adjustments a **covenant cap** clamps the final score to ≤ `covenant_score_cap` (default 60) when current DSCR is below the per-type covenant. CMHC MLI-eligible records also carry a **dual score** — ranked on MLI Standard, with an MLI Select upside score + gap alongside. Weights and floor/ceiling thresholds are configurable and persisted in `json/score_weights.json`. Also provides `solve_targets()` — binary search over each lever (asking price, rent, interest rate, down payment) to find the value that would push the score to ≥ 99.5/100. |
| `city_ranker.py` | **`CityRanker`** — groups scored properties by city and computes per-city signals (avg score, best score, volume, cap rate, price drop, DOM, CoCR). Metric factors are **normalized per listing, then averaged** (not averaged then normalized — the ramp's clamp is non-linear, so the order matters), applies configurable city-level signal weights, and reports how many active listings are still awaiting re-analysis (`pending_reanalysis`). |
...
| `score_weights.json` | Scoring weights (active, non-zero weights are renormalized to sum to 1.0 — DSCR, Cash Flow, Price Drop and DOM ship at zero, see "Market/listing signals" and the scorer notes above), floor/ceiling thresholds per metric (CoCR and Cash-Flow ramps extend below zero; Cash Flow is scored as % of price), city-level signal weights, the confidence smoothing constant `k`, and `covenant_score_cap` (the below-covenant score ceiling, default 60). |
...
| **Financing Robustness** | A scored 0–100 factor and card panel measuring how much financing stress a deal absorbs. **Rate Risk** (scheduled/certain — the term matures and renews at market) and **Amortization Risk** (conditional — only on a forced refinance) are reported as two separate readings, each to two thresholds: **covenant breach** (DSCR falls to the per-type covenant) and **cash-flow zero** (DSCR 1.00). The gap between them is itself the signal — covenant breach usually arrives first, while cash flow is still positive (the lender acts before the owner feels pain). Threshold-independent: "+180 bps" is an absolute fact, comparable across asset classes. The sub-score weights rate margin more heavily (0.7/0.3); refs live in `config/underwriting.json`. Takes the scoring weight vacated by the now-display-only DSCR and Cash Flow. |
| **Break-Even Financing** | The financing terms a deal needs to reach positive cash flow and DSCR ≥ its per-type covenant (e.g. "DSCR≥1.25 up to 7.71% · cash-flow-positive up to 10.54% at 25yr"), plus a verdict: passes at current terms / conditional / **fails under all realistic terms**. Diagnostic only — not a score input. |
| **Covenant score cap** | A post-hoc gate: when a property's current (unstressed) DSCR is below its per-type `covenant_dscr`, its final score is capped at `covenant_score_cap` (config, default 60) — covenant compliance is a gate, not a factor that strong projected returns can outweigh. Applied after all other adjustments; floors nothing; skipped when DSCR is unavailable (no-debt / pending) so a missing value never triggers a false cap. Not a return to weighted DSCR scoring. |
...
| **CMHC MLI panel** | Rendered only when the deal is MLI-routed — 5+ residential units, or mixed-use with commercial GFA ≤30% (share = 1/floors, equal-plate convention). Shows **MLI Standard** (85% LTV / 40yr / DSCR 1.20 — the ranking basis) and **MLI Select** (85% acquisition cap / point-tier amortization modelled at 45yr / DSCR 1.10 — upside). 95% LTV is labelled *unverified upside* (requires 100pts + DSCR≥1.20 + full lender advance). No MLI rows appear on conventional deals. |
| **MLI dual score** | MLI-eligible deals are ranked on the **MLI Standard** score and carry an **MLI Select** upside score + the gap. Because acquisition LTV is capped at 85% for both programs, equity/loan are identical, so the Select edge is amortization (45 vs 40yr) and the 1.10 DSCR floor — not leverage — and the gap is small by construction. Shown on the card as "MLI Select N (+gap)". |
| **Small-balance floor** | Uniform across all MLI-routed types: a deal whose **75%-LTV loan** (`price × 0.75`, not price) is below `min_practical_loan` (~$1M) finances conventionally (75% LTV / 25yr), not MLI — an MLI lender declines a sub-floor balance regardless of asset type. No deal of any type gets MLI terms on a sub-$1M loan. |
| **Mixed-use 30% rule** | A mixed-use building is CMHC MLI-eligible only when commercial gross floor area ≤30% **and** it has ≥5 residential units. Commercial share is `1 ÷ floor_count` under the equal-plate convention (ground-floor commercial, upper floors residential), gated flat at 30% — 4+ floors (≤25%) qualify, 3 floors (33.3%) do not. |
