"""Tests for realtor.ca query building and price parsing (no network)."""
from scraping.realtor_scraper import build_query, _parse_price


def test_query_appends_province_when_missing():
    # Stored address includes the city but not the province.
...
def test_query_strips_trailing_province_from_address_then_readds():
    # _display_address strips a trailing ", ON"; province is re-added once.
