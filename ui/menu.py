...
class PropertyMenu(RateEditorMixin, ConfigEditorMixin, CsvHandlerMixin):
    ...
    def _edit(self):
        ...
        while True:
            ...
            # Down payment / rate / amortization resolve automatically from the
            # per-type financing config at analysis time — not shown or edited here.
        ...
        if todo:
            ...
            try:
                with RealtorScraper() as scraper:
                    ...
                    print("\n  Opening the search engine in the browser window...")
                    if scraper.open_home():
                        input("  The search engine is challenging the browser. In "
                              "the window, solve any\n  CAPTCHA and accept cookies, "
    ...
    def _build_report_row(p: dict, scored: dict, targets: dict) -> dict:
        return {
            ...
            "dom_band":     scored.get("dom_band"),
            "price_drop_band": scored.get("price_drop_band"),
            "liquidity_band":  scored.get("liquidity_band"),
            "market_signal_multiplier": scored.get("market_signal_multiplier"),
            "deal_context_read": scored.get("deal_context_read"),
            "verified_income_pct": scored.get("verified_income_pct"),
    ...
    def _prompt_property(self) -> PropertyInput:
        ...
        # Down payment, interest rate, and amortization resolve from the global
        # financing config BY PROPERTY TYPE (+ unit count for the multi-family
        # 1-4/5+ split); hold period is house-wide. Not entered per listing and
        # not shown here — edit the type blocks / defaults via config (defaults
        # via main menu 'f').
        fin              = resolve_financing(original_price, property_type_raw, total_units)
    ...
    def _record_to_prop(p: dict) -> PropertyInput:
        ...
        # Down payment, interest rate, and amortization resolve from the global
        # financing config BY PROPERTY TYPE (+ unit count for the multi-family
        # 1-4/5+ split); hold period is house-wide. Every re-analysis picks up the
